...
async def show_start_message(message: Message, person, lang):
    ...
    log.debug(f"User {person.tgid}: Retrieved {len(keys)} keys from database: {[k.id for k in keys]}")
    for key in keys:
        server_id = key.server_table.id if key.server_table else None
        log.debug(f"Key {key.id}: trial_period={key.trial_period}, free_key={key.free_key}, subscription={key.subscription}, server_id={server_id}")
    
    valid_keys = keys  # Сохраняем все ключи, включая неактивные
    log.debug(f"User {person.tgid}: Valid keys after filtering: {len(valid_keys)}")
    ...
    if valid_keys:
        ...
        for key in valid_keys:
            ...
            if key.trial_period:
                ...
                key_label = "Бесплатный "
            elif key.server_table:  # Если есть привязка к серверу, считаем, что ключ выдан админом
                key_label = "Выданный админом "
            else:
                key_label = ""
            if time_from_db > current_time:
                days = (time_from_db - current_time).days
                text += f'\n🔑 {key_label}🟢 Ключ №{count_key} до {time_from_db.strftime("%d-%m-%Y")}'
            else:
                has_expired = True
                text += f'\n🔑 {key_label}🔴 Ключ №{count_key} (Неактивен, можно продлить)'
            count_key += 1
        if has_expired:
            text = '🚨 Ваша подписка на один из ключей закончилась!🔒\n' + \
                   'Если вы хотите продолжить пользоваться нашими услугами, пожалуйста, продлите подписку. 😊\n\n' + text
    else:
        text += '\n' + _('no_active_subscriptions', lang)
        log.debug(f"User {person.tgid}: No valid keys found to display")
...
async def format_keys_message(keys, lang, is_detail=False, selected_key_id=None):
    ...
    if is_detail and selected_key_id is not None:
        ...
        status = "🟢" if end_date > current_time else "🔴"
        end_date_str = end_date.strftime("%d-%m-%Y")
        # Определяем тип ключа
        if selected_key.trial_period:
            key_label = "Пробный "
        elif selected_key.free_key:
            key_label = "Бесплатный "
        elif selected_key.server_table:
            key_label = "Выданный админом "
        else:
            key_label = ""
        if end_date > current_time:
            ...
            keys_text += f"{key_label}{status} Ключ {key_number} (Неактивен, можно продлить) (выбранный ключ)\n"
    else:
        count_key = 1
        for key in keys:
            ...
            if end_date > current_time:
                ...
                keys_text += f"{key_label}{status} Ключ {count_key} (Неактивен, можно продлить)\n"
            count_key += 1
...
async def show_key_details(
    ...
    try:
        async with AsyncSession(autoflush=False, bind=engine()) as db:
            ...
            if config is None:
                log.error(f"Failed to generate config for key {callback_data.key_id}")
                await callback.message.answer(
                    text=_('server_not_connected', lang),
                    parse_mode="HTML"
                )
            ...
            log.info(f"Successfully showed key details and config for user {callback.from_user.id}, key_id={callback_data.key_id}")
...
async def generate_new_key(callback: CallbackQuery, state: FSMContext):
    lang = await get_lang(callback.from_user.id, state)
    log.debug(f"Returning to main menu for user {callback.from_user.id} (generate_new_key called)")
    person = await get_person(callback.from_user.id)
    await show_start_message(callback.message, person, lang)
    await callback.answer()
        ...
        if (
            ...
            for admin_id in CONFIG.admin_tg_ids:
                try:
                    ...
                    log.error(f"Unexpected error sending message to admin {admin_id}: {e}")
