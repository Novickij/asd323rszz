...
async def input_name(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(_('input_percent_message', lang))
...
async def input_time_promo(message: Message, state: FSMContext):
    lang = await get_lang(message.from_user.id, state)
    try:
        try:
...
async def callback_show_promo(call: CallbackQuery, state: FSMContext):
    ...
    for promo in all_promo:
        text_server = Text(
            Code(promo.text),
            f' | {promo.percent} %'
            f' - ', Bold(f'{len(promo.person)}/{promo.count_use}'), ' 🏷'
        )
        await call.message.answer(
