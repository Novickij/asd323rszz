...
from bot.database.methods.get import _get_person, _get_server, get_server_id
from bot.database.models.main import Persons, WithdrawalRequests, Keys, PromoCode, message_button_association, Location, Vds
...
async def add_referral_popup_person(amount, tgid):
    async with AsyncSession(engine) as db:
        person = await _get_person(db, tgid)
        if person is not None:
            person.referral_balance += int(amount)
...
async def person_special_offer(telegram_id):
    async with AsyncSession(engine) as db:
        person = await _get_person(db, telegram_id)
        if person is not None:
...
async def add_pomo_code(tgid, promo_code: PromoCode):
    async with AsyncSession(engine) as db:
        async with db.begin():
...
async def successful_application(id_application):
    async with AsyncSession(engine) as db:
...
async def succes_aplication(id_application):
    """
    Alias for successful_application for backward compatibility.
    """
    return await successful_application(id_application)
...
async def update_delete_users_server(telegram_id):
    """
    Alias for update_key_users_server for backward compatibility.
    """
    return await update_key_users_server(telegram_id)
...
async def add_time_key(key_id, time_sub, id_payment=None):
    async with AsyncSession(engine) as db:
        ...
        if key is not None:
            ...
            # Если ключ истёк, устанавливаем новую дату от текущего момента
            if key.subscription < current_time:
                key.subscription = current_time + time_sub
            else:
                key.subscription += time_sub
            key.id_payment = id_payment
            key.notification_sent = False
            key.notion_oneday = False
            if key.trial_period:
                key.trial_period = False
            if key.free_key:
                key.free_key = False
            # Включаем клиента в X-UI
            if key.server is not None:
                try:
                    ...
                    if server:
                        ...
                        success = await server_manager.enable_client(key.user_tgid, key.id)
                        if success:
                            log.info(f"Enabled key {key.id} (user {key.user_tgid}, email {email}) on server {key.server}")
                        else:
                            log.warning(f"Failed to enable key {key.id} (email {email}) on server {key.server}")
                    else:
                        log.warning(f"Server {key.server} not found for key {key.id}")
                except Exception as e:
                    log.error(f"Failed to enable key {key.id}: {e}")
            await db.commit()
            return True
        return False
