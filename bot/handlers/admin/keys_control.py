...
from bot.misc.loop import disable_key, delete_key  # Исправляем импорт: добавляем disable_key и оставляем delete_key
...
async def edit_key_actions(message: Message, state: FSMContext) -> None:
    ...
    if action == 'delete_key':
        try:
            await delete_key(key_id)  # Используем delete_key для явного удаления админом
        except Exception as e:
