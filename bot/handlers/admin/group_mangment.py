...
from bot.database.methods.get import (
    ...
    get_users_group,
    get_person_id,
    get_group, get_group_name, get_count_groups, get_server_id
)
...
async def add_group_users(message: Message, state: FSMContext) -> None:
    ...
    users_id = []
    for user in users:
        await delete_users_server(message, user, lang)
...
async def delete_users_server(message, user, lang):
    for key in user.keys:
        ...
        if key.server is not None:
            ...
            try:
                ...
                await message.answer(_('error_delete_all_users_server', lang))
                return False
    await update_key_users_server(user.tgid)
    return True
